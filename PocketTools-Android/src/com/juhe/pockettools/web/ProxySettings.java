...
public class ProxySettings {
	...
	private static boolean isSystemProxyValid(Context ctx) {
		...
		_proxyString = Settings.Secure.getString(_contentResolver,
				Settings.Secure.HTTP_PROXY);
		if (_proxyString != null && _proxyString != ""
				&& _proxyString.contains(":")) {
			return true;
		} else {
			return false;
		}
	...
	private static boolean isInternetReachable(Context ctx) {
		...
		HttpHost proxy = new HttpHost(getSystemProxyAddress(ctx),
				getSystemProxyPort(ctx));
		...
		try {
			...
			Log.d("ProxySettings", "Is internet reachable : "
	...
	// _proxyStatus = false;
	// message = ctx.getResources().getString(
	// R.string.ProxySettings_ErrorProxyServerNotReachable);
	...
	public static void setSystemProxy(Context ctx) {
		// Toast.makeText(ctx,
		// ctx.getResources().getString(R.string.ProxySettings_EnablingProxySettings),
		...
		if (_proxyStatus) {
			...
			// Toast.makeText(ctx,
			// ctx.getResources().getString(R.string.ProxySettings_ProxySettingsEnabled),
	...
	public static void resetSystemProxy(Context ctx) {
		if (_proxyStatus != null && _proxyStatus == true) {
			try {
				...
				Log.e("ProxySettings",
						"Exception resetting WebKit proxy settings: "
								+ e.toString());
			}
		}
	}
	...
	private static boolean setProxy(Context ctx, String host, int port) {
		...
		try {
			...
			Log.e("ProxySettings", "Exception setting WebKit proxy settings: "
					+ e.toString());
		}
		return ret;
	}
	...
	private static Object GetNetworkInstance(Context ctx)
